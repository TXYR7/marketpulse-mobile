...
38 项冒烟断言：个股相似案例形状、决策助手三层输出、mergePools 三池部分失败降级、封单星级、情绪三指标（市场断板率/昨首板溢价/昨高位溢价）、题材升降方向、晋级八维硬否决、自由文本路由、sw.js↔version.js 版本一致性、**整文件语法护栏**（7 个运行时 JS 全部经 vm.SourceTextModule 编译，改坏语法会被抓住）。
